...
## Test 2 result (PuTTY capture 2026-09-23 19:58)

The user reports that the game worked after one rest/wake cycle. In the capture, suspend ends before resume phase 4 at line 928, then the event changes from 300 to WORKING (1000) at line 1208. The monitor waits, starts the A53/PPR check at line 1540, receives install result 0 at line 1820, and reports patch verified at line 1824. PPSA03530 launches at line 1891 and its eboot executes at line 2064. The capture contains no `errno=5`/`0x2c` I/O failures. It does show an IOD `0x3e` error with automatic retry to errno 0 during suspend; this is distinct from the persistent errors in test 1. A second suspend starts near line 5114, but the file ends before another wake, so repeat-cycle behavior is unverified. The resumed A53/PPR operation took 15,714 ms, in addition to the monitor's stable/idle wait.
